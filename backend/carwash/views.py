...
from .serializers import (
    ...
    CarWashPostPatchSerializer,
    CarWashReviewListSerializer,
    CarWashReviewPostPatchSerializer,
    CarWashUpdateRequestSerializer,
    PaymentStatusSerializer,
    PreSignedUrlSerializer,
    WashTypeSerializer,
    AmenitySerializer,
    OfferSerializer,
    CarWashCodeSerializer,
    OfferCreatePatchSerializer,
    CarWashCodeCreatePatchSerializer,
...
from .filters import (
    DynamicSearchFilter,
    ListCarWashFilter,
    ListCarWashReviewFilter,
    ListCarWashCodeFilter,
    ListOfferFilter,
)
...
class CarWashCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        data = {
            ...
            "payouts_status": (
                "PENDING"
                if self.request.data.get("is_bounty_claim", False)
                else "NOT_APPLICABLE"
            ),
        }
...
class CarWashUpdateView(generics.UpdateAPIView):
    ...
    def perform_update(self, serializer):
        try:
            ...
            raise DRFValidationError({"error": "is_bounty_claim boolean is required."})
        try:
            payment_method = None
            payment_handle = None
            if is_bounty_claim:
    ...
    def get_queryset(self):
        ...
        if user_lat and user_lng:
            ...
            queryset = queryset.annotate(
                distance=Coalesce(
                    Distance("location", reference_point, output_field=FloatField())
                    * Value(0.000621371, output_field=FloatField()),
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                signed_url_object = supabase_client.storage.from_(
...
@extend_schema(
    ...
    examples=[
        OpenApiExample(
            "One-Time Offer Example", value={"code": "WASH123", "offer_id": 1}
        )
    ],
)
class CarWashCodeMarkAsUsedView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        code = CarWashCode.objects.filter(
            code=serializer.validated_data["code"], offer=offer_id, user__isnull=True
        ).first()
        if not code:
        ...
        if offer.offer_type in ["TIME_DEPENDENT", "GEOGRAPHICAL"]:
            ...
            existing_usage = CarWashCode.objects.filter(
                offer=offer_id, user=request.user
            )
...
class ListOfferAPIView(DynamicFieldsViewMixin, ListAPIView):
    ...
    def debug_offer_availability(self, package_id=None):
        """
        Debug method to check why offers might not be returned
        """
        debug_info = {
            "timestamp": timezone.now().isoformat(),
            "user_authenticated": bool(self.request.headers.get("Authorization")),
            "user_lat": self.request.GET.get("userLat"),
            "user_lng": self.request.GET.get("userLng"),
        ...
        if package_id:
            try:
                package = CarWashPackage.objects.get(id=package_id)
                debug_info["package"] = {
                    "id": package.id,
                    "name": package.name,
                    "car_wash_name": package.car_wash.car_wash_name,
                    "has_offer": hasattr(package, "offer"),
                }

                if hasattr(package, "offer"):
                    offer = package.offer
                    codes_count = offer.codes.count()
                    unused_codes_count = offer.codes.filter(user__isnull=True).count()
                    ...
                    debug_info["offer"] = {
                        ...
                        "start_time": (
                            offer.start_time.isoformat() if offer.start_time else None
                        ),
                        "end_time": (
                            offer.end_time.isoformat() if offer.end_time else None
                        ),
                        "radius_miles": (
                            float(offer.radius_miles) if offer.radius_miles else None
                        ),
                    }

                    # Check time validity for TIME_DEPENDENT offers
                    if offer.offer_type == "TIME_DEPENDENT":
                        now = timezone.now().time()
                        is_time_valid = offer.start_time <= now <= offer.end_time
                        debug_info["offer"]["is_time_valid"] = is_time_valid
                        debug_info["offer"]["time_check"] = {
                    ...
                    # Check geographical distance for GEOGRAPHICAL offers
                    if (
                        offer.offer_type == "GEOGRAPHICAL"
                        and debug_info["user_lat"]
                        and debug_info["user_lng"]
                    ):
                        user_location = Point(
                            float(debug_info["user_lng"]),
                            float(debug_info["user_lat"]),
                            srid=4326,
                        )
                        distance_query = (
                            Offer.objects.filter(id=offer.id)
                            .annotate(
                                distance=Distance(
                                    "package__car_wash__location", user_location
                                )
                            )
                            .first()
                        )
                        ...
                        if distance_query:
                            distance_miles = (
                                distance_query.distance.mi
                                if distance_query.distance
                                else None
                            )
                            is_within_radius = (
                                distance_miles <= float(offer.radius_miles)
                                if distance_miles and offer.radius_miles
                                else False
                            )

                            debug_info["offer"]["geographical_check"] = {
                                "distance_miles": distance_miles,
                                "radius_miles": float(offer.radius_miles),
                                "within_radius": is_within_radius,
                            }

                    # Check if user has used ONE_TIME offers
                    if (
                        self.request.headers.get("Authorization")
                        and offer.offer_type == "ONE_TIME"
                    ):
                        user_used_codes = offer.codes.filter(
                            user=self.request.user
                        ).count()
                        debug_info["offer"]["user_used_codes"] = user_used_codes
                        debug_info["offer"]["will_be_excluded"] = user_used_codes > 0

            except CarWashPackage.DoesNotExist:
                debug_info["error"] = f"Package with id {package_id} not found"
            except Exception as e:
                debug_info["error"] = str(e)

        return debug_info

    def debug_carwash_offers(self, car_wash_id=None):
        """
        Debug method to check all offers for a car wash
        """
        debug_info = {
            "timestamp": timezone.now().isoformat(),
            "user_authenticated": bool(self.request.headers.get("Authorization")),
        ...
        if car_wash_id:
            try:
                ...
                debug_info["car_wash"] = {
                    "id": car_wash.id,
                    "name": car_wash.car_wash_name,
                    "packages_count": car_wash.packages.count(),
                }

                packages = car_wash.packages.all()
                packages_debug = []
                ...
                for package in packages:
                    ...
                    if hasattr(package, "offer"):
                        offer = package.offer
                        codes_count = offer.codes.count()
                        unused_codes_count = offer.codes.filter(
                            user__isnull=True
                        ).count()
                        ...
                        package_debug["offer"] = {
                            ...
                            "total_codes_count": codes_count,
                            "unused_codes_count": unused_codes_count,
                            "will_appear_in_search": unused_codes_count > 0,
                        }
                        ...
                        if offer.offer_type == "TIME_DEPENDENT":
                            now = timezone.now().time()
                            is_time_valid = offer.start_time <= now <= offer.end_time
                            package_debug["offer"]["is_time_valid"] = is_time_valid
                            package_debug["offer"]["time_check"] = {
                                "current_time": now.isoformat(),
                                "start_time": offer.start_time.isoformat(),
                                "end_time": offer.end_time.isoformat(),
                                "valid": is_time_valid,
                    ...
                    packages_debug.append(package_debug)

                debug_info["packages"] = packages_debug

            except CarWash.DoesNotExist:
                debug_info["error"] = f"Car wash with id {car_wash_id} not found"
            except Exception as e:
                debug_info["error"] = str(e)
    ...
    def get_queryset(self):
        ...
        if not self.request.headers.get("Authorization"):
            ...
            queryset = queryset.annotate(
                ...
                ~Q(codes_count=0), offer_type__in=["TIME_DEPENDENT", "GEOGRAPHICAL"]
            )
            ...
            queryset = queryset.annotate(
                is_time_valid=Case(
                    ...
                    When(offer_type="TIME_DEPENDENT", then=Value(False)),
                    default=Value(True),
            ...
            if user_lat and user_lng:
                ...
                queryset = queryset.annotate(
                    ...
                    Q(
                        offer_type="GEOGRAPHICAL",
                        distance__lte=F("radius_miles") * 1609.34,
            ...
            queryset = queryset.annotate(
                codes_count=Count("codes", filter=~Q(codes__user__isnull=False))
            ).filter(~Q(codes_count=0))

            # Filter out one-time offers that have been used by the user
            queryset = queryset.exclude(
                codes__user=self.request.user, offer_type="ONE_TIME"
            )
            ...
            if user_lat and user_lng:
                ...
                queryset = queryset.filter(
                    offer_type__in=["TIME_DEPENDENT", "ONE_TIME"]
                )
...
class CheckPaymentStatusView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        payment = (
            self.get_queryset()
            .filter(payment_intent_id=kwargs["payment_intent_id"], user=request.user)
            .first()
        )
        if not payment:
            return Response(
...
class UserPaymentHistoryView(DynamicFieldsViewMixin, ListAPIView):
    ...
    def get_queryset(self):
        return (
            ...
            .order_by("-created_at")
            .select_related(
                "offer", "offer__package", "offer__package__car_wash", "carwash_code"
            )
        )
...
def get_packages(request):
    ...
    packages = CarWashPackage.objects.filter(car_wash_id=car_wash_id).values(
        "id", "name"
    )
